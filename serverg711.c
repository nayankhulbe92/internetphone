...
void periodic_task(int signum)
{
    ...
    uint8_t outbuff[BUFSIZE];             // must be an array
    size_t i;
    uint8_t temp_8;
    uint8_t temp_16;
...
for (i=0; i < BUFSIZE; ++i)               // only the data actually read
        {
            temp_8 = bufi[i];
            temp_16 = Snack_Alaw2Lin(temp_8);
            outbuff[i] = temp_16;
        }

if (pa_simple_write(saa,outbuff, (size_t) r, &error) < 0) {//play the contents of "bufi" recieved from client
